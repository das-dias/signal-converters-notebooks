...
windows = {
    "rectangular": lambda x, n: x,
    "hanning": lambda x, n: x * hann(n),
    "hamming": lambda x, n: x * hamming(n),
    "blackmanharris": lambda x, n: x * blackmanharris(n),
    "blackman": lambda x, n: x * blackman(n),
    "gaussian": lambda x, n: x * gaussian(n),
    "kaiser": lambda x, n: x * kaiser(n),
    "cosine": lambda x, n: x * cosine(n),
    "parzen": lambda x, n: x * parzen(n),
}
...
class FFTEngine:
    ...
    def __init__(
        ...
        self.snr: float = None
        self.sfdr: float = None
        self.sndr: float = None
        self.thd: float = None
        self.enob: float = None
        self.h2: float = None
        self.h3: float = None
        self.dc: float = None
    ...
    def fft(
        ...
        """Compute the FFT of the stored signal.
        Args:
            n_points (int): Number of points to use in the FFT.
